...
const Chart6Component: React.FC = () => {
  const [chart6Data, setChart6Data] = useState<Chart6[]>([]);
  const [editDialogOpen, setEditDialogOpen] = useState(false);
  const [currentEditItem, setCurrentEditItem] = useState<Chart6 | null>(null);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response6 = await client.models.Chart6.list();
        const sortedData6 = response6.data
          .map(item => ({
            id: item.id ?? '',
            Order: item.Order ?? 0,
            Title: item.Title ?? '',
  ...
  const handleEditClick = (item: Chart6) => {
  ...
  const handleSave = async () => {
    if (currentEditItem) {
      try {
        await client.models.Chart6.update(currentEditItem);
        setChart6Data(prevData =>
...
export default Chart6Component;